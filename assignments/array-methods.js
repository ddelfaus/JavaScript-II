...
firstNamesAllCaps = runners.map(names => names.first_name.toUpperCase());
...
runnersLargeSizeShirt = runners.filter(function(size){
...
runnersSmallSizeShirt = runners.filter(function(size){
  return size.shirt_size ==='S';
})

console.log('small shirt size')
console.log(runnersSmallSizeShirt);

// Problem 2
let idnumbers = [];
runners.forEach(function(idnum){
  return idnumbers.push(`${idnum.id} `);
})
...
lastNamesAllCaps = runners.map(names => names.last_name.toUpperCase());

console.log(lastNamesAllCaps);
lastNamesAllCaps.sort();
console.log(lastNamesAllCaps);